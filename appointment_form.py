...
class DEL_AP:
    ...
    def DELETE_AP(self):        
        ...
        mycursor = conn.cursor()
        sql_check = "SELECT * FROM appointment WHERE APPOINTMENT_NUMBER= %s"
        mycursor.execute(sql_check, (inp_d,))
        result = mycursor.fetchone()
        if not result:
            tkinter.messagebox.showerror("HOSPITAL DATABSE SYSTEM", "PATIENT APPOINTMENT NOT FIXED")     
        else:
            mycursor.execute('DELETE FROM APPOINTMENT where APPOINTMENT_NUMBER=%s',(inp_d ,))
    ...
    def SEARCH_AP(self):
        ...
        if not result:
            ...
            for i in row:
                ...
                self.l3 = Label(self.LoginFrame,text="APPOINTMENT TIME",font="Helvetica 14 bold",bg="cadet blue",bd=22)
